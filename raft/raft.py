...
class RaftNode(threading.Thread):
    def __init__(self, config, name, role='follower', verbose=False):
        ...
        # Timing variables
        self.election_timeout = 0.3 # Failed vote backoff, used for pretty much all timing related things.
        ...
        self.log = [{'term': 1, 'entry': 'Demo Entry', 'id': -1}]           # Your log. Log entries are a dict with the following fields: term, entry, id.
        self.log_hash = {}
        self.quorum = (self.current_num_nodes - 1) // 3 * 2 + 1
        # self.quorum = self.current_num_nodes // 2
    ...
    def client_request(self, value, start, id_num=-1):
        '''
    ...
    def _follower(self):
        ...
        while ((not self._terminate) and (self.check_role() == 'follower')):
            ...
            if (incoming_message is not None):
                ...
                # print("Follower recv message: ", self.my_id, incoming_message.entries)
                ...
                if (incoming_message.direction == MessageDirection.Request):
                    ...
                    # Incoming message is a heartbeat, make sure you're up to date, restart the timer
                        ...
                        if (self.leader_id):
                            ...
                            if (client_request is not None):
                                # print("Follower sending message: ", client_request)
                    ...
                    # Incoming message is some data to append
                        ...
                        # Reply false if log doesnt contain an entry at prev_log_index whose term matches prev_log_term
                            ...
                            print("log doesnt contain an entry at prev_log_index whose term matches prev_log_term")
                            self._send_acknowledge(incoming_message.leader_id, False)
                    ...
                    # Incoming message is a commit message
                        ...
                        # end_time = time.time()
                        # time_taken = self.start_time - end_time
                        # print("Committing:", time_taken)
    ...
    def _leader(self):
        ...
        while ((not self._terminate) and (self.check_role() == 'leader')):
            ...
            incoming_message = self._get_message()
            if (incoming_message is not None):
                # print("Leader recv message: ", incoming_message)
                ...
                if (incoming_message.direction == MessageDirection.Response):
                        ...
                        if (self.next_index[sender_index] is not None):
                            ...
                            for index in log_lengths:
                                ...
                                if (replicated_on >= (self.current_num_nodes // 2 + 1)):
                                    ...
                                    # print("Append time: ", str(time_taken))
                    ...
                    # If its a client then it's a new request
                        ...
                        # print("Leader Sending message : ", client_request)
    ...
    def _broadcast_append_entries(self, entry):
        ...
        for node, index in enumerate(self.next_index):
            ...
            if (index is None):
                # print("Sending to: ", entry, self.all_ids[node])
    ...
    def _broadcast_commmit_entries(self, index):
        ...
        # end_time = time.time()
        # time_taken = end_time - self.start_time
        # print("Sent commits time:", str(time_taken))
    ...
    def _send_committal(self, index, receiver=None):
        ...
        # print("Commit time: ", str(time_taken))
        self._send_message(message)
